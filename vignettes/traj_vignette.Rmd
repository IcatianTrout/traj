...
Since we set to `NULL` the `nclusters` argument , the algorithm of Tibshirani et al. based on the GAP statistic was used to select the optimal number of clusters. Note that, by default, the argument `K.max` is set to 8, meaning that the optimal number of clusters between 1 and 8 is chosen. In the present case, the algorithm judged that the optimal number $k$ of clusters is 7 because this corresponds to the "elbow" in the graph of the GAP statistic, in the precise sense that $k=7$ is the first value of the argument in the function $GAP(k)$ for which the interval $GAP(k)\pm SE(k)$ overlaps with the interval $GAP(k+1)\pm SE(k+1)$. This can be inspected visually with the input
...
To visually inspect the classification, we write `plot(step3, ask = TRUE)`.
...
By visual inspection, it is clear that clusters 1 & 4, 3 & 6 as well as 5 & 7 are more similar than dissimilar. In fact, it can be checked that clusters 1 & 4 contain 83% of group D, cluster 2 contains 100% of group C, clusters 3 & 6 contain 90% of group A and clusters 5 & 7 contain 100% of group D.
...
In the "Loadings" table, the measure selected to represent RC2 (the second rotated component) is measure 17 because it has a Pearson correlation of 0.965 with it. It is notable that measure 13 and measure 14 also have a high correlation with RC2 (0.936 and -0.912 respectively). The measure selected to represent RC1 is measure 6 with a correlation of 0.913 but we note that measures 1, 2 and 3 have comparable correlations with RC1 (0.896, 0.899 and 0.911 respectively). The selected representative of RC3 is measure 12 with a correlation 0.872 but measure 11 and, to a lesser extent, measure 5 also are highly correlated with RC3 (0.8111 and 0.784 respectively). The selected representative of RC4 is measure 9 and there really aren't other contenders.
...
```{r ex1.step3f}
step2.bis <- Step2Selection(trajMeasures = step1, select = c(13, 1, 5, 9)) 
step3.bis <- Step3Clusters(trajSelection = step2.bis, nclusters = 7)
```
...
-   **m9: Proportion of time spent below the mean.**
    ...
    This measure has the potential to detect the presence of thin spikes because, the mean of $y_0$ being 0, if $T^-$ (the time spent below the mean) is small compared to $T^+$ (the time spent above the mean), the implication is that $y_0$ ventures *farther* away from the mean when it is negative than when it is positive. Thus, a small m9 is indicative of sharp negative spikes, while a large m9 is indicative of sharp positive spikes. Note that if both positive and negative spikes are present in equal amount, m9 will not detect them.
...
-   **m18: Early change/Later change.** Given an observation time $t_m$ with $1<m<N$ which is to act as the "midpoint" of the trajectory, this measure compares the net change in the first half of the trajectory to the net change happening in the later half:$$\mathrm{m18} = \frac{y(t_m)-y(t_1)}{y(t_N)-y(t_m)}.$$
...
[2] Nishiyama T. Improved Chebyshev inequality: new probability bounds with known supremum of PDF [arXiv:1808.10770v2](https://arxiv.org/abs/1808.10770v2)
