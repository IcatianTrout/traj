...
The `traj` package implements a clustering algorithm for functional data (henceforth referred to as trajectories) that extends previous work from Leffondré et al [1]. This algorithm is comprised of three steps. The first step sumarizes the main features of the trajectories by computing the 18 measures listed below and detailed in Appendix A.
...
2.  A principal component analysis is conducted on the measures. The main principal components (defined as those principal components contributing more to the total variance than any of the individual standardized measures) are selected.
...
Two outlying values were detected for measure m18 for trajectories ID23 and ID110 and were replaced by more likely values according to the distribution of m18. Once the measures are computed, we use the `Step2Selection` function to extract the measures that best characterize the trajectories.
...
Two measures are defined as '"perfectly or almost perfectly correlated" if the absolute value of their Pearson correlation coefficient is greater than 0.98. The print function provides more detailed informations:

```{r ex1.step2b}
print(step2)
```

Measure 4 (Standard deviation) was dropped because it is perfectly or almost perfectly correlated with measure 2 (Range) and measure 12 (Mean of the first derivative) was dropped because it is perfectly or almost perfectly correlated with measure 5 (Slope of the linear model). The `Step3Clusters` function uses the k-means algorithm (function `stats:::kmeans`) on the measures selected in step 2 to cluster the trajectories.
...
By setting `nclusters` to `NULL`, `Step3Clusters` uses the criterion proposed by Tibshirani et al. [2] to select the optimal number of clusters. In our case, this gives $k=4$ as can be inspected visually with
...
In a recent paper published on the arXiv [3], the author proves that for any *continuous* random variable $X$ with finite first two moments there holds $$
